...
origin2=origin[::-1]
play=True
def start_enc_dec(origin):
    word_to_enc=input('enter your message :\n')
    encoded_list=[]
    encode_by=int(input('entre your shift number \n'))
    for j in range(len(word_to_enc)) :
            for i in range(len(origin)):
                if word_to_enc[j] not in origin:
                    print(f'{word_to_enc[j]} is not in {origin}')
                    encoded_list.append(word_to_enc[j])
                    break
                elif word_to_enc[j]==origin[i]:
                    if i+encode_by<=25:
...
while play:
    ...
    if choisc=='e':
        start_enc_dec(origin=origin)
        play=play_again()
    if choisc=='d':
        start_enc_dec(origin=origin2)
        play=play_again()